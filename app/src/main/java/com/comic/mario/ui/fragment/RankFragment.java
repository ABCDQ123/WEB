...
import com.comic.mario.ui.MainActivity;
...
public class RankFragment extends LazyFragment implements MutiViewHolder, ImpKiKyo {
    ...
    private KiKyoRankClient kiKyoRankClient;

    @Override
    public void onLazyLoad() {
    }

    public static RankFragment newInstance(Serializable obj) {
    ...
    public void onDestroy() {
        super.onDestroy();
        if (null != kiKyoRankClient) {
            kiKyoRankClient.cancel();
        }
    }
    ...
    private void init() {
        ...
        rv_rank_fragment.setLoadMoreListener(() -> {
            rv_rank_fragment.setLoading(true);
            kiKyoRankClient.next();
        });

        kiKyoRankClient = new KiKyoRankClient();
        kiKyoRankClient.request(getContext(), items, mRankBean, this);
    }
    ...
    public void onBindViewHolder(int tag, RecyclerView.ViewHolder holder, int position) {
        if (tag == 1) {
            ...
            holder.itemView.setOnClickListener(v -> {
                ((MainActivity) getActivity()).startActivity(dataBean.getLink());
            });
        }
    }
